...
from .models_det import compile_model
...
def train(
    ...
    while True:
        ...
        for batchi, ret in enumerate(train_loader):
            ...
            l_hm, l_wh, l_off = lbx['hm_loss'], lbx['wh_loss'], lbx['off_loss']

            # print(ret['hm'].shape)
            # hmIM = torch.sum(ret['hm'], dim=1, keepdim=True)
            # hmIM = hmIM[0, 0].detach().cpu().numpy()
            # from PIL import Image
            # print(np.min(hmIM), np.max(hmIM))
            # Image.fromarray(np.uint8(hmIM*255)).show()
            # raise None
            ...
            hm_loss.update(l_hm.item(), ret['img'].size(0))
            wh_loss.update(l_wh.item(), ret['img'].size(0))
            off_loss.update(l_off.item(), ret['img'].size(0))
            ...
            if counter % 10 == 0:
                writer.add_scalar('train/l_box', l_box, counter)
                writer.add_scalar('train/l_hm', l_hm, counter)
                writer.add_scalar('train/l_wh', l_wh, counter)
                writer.add_scalar('train/l_off', l_off, counter)
                writer.add_scalar('train/lr', opt.param_groups[0]['lr'], counter)

        print(f'epoch-{epoch:3d}|{time() - t0:.3f}, '
              f'box:{box_loss.avg:.4f}, hm:{hm_loss.avg:.4f}, '
              f'wh:{wh_loss.avg:.4f}, '
              f'off:{off_loss.avg:.4f}')

        # LR plateaued

        scheduler.step(box_loss.avg)
