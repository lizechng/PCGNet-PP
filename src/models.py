...
class RadarPts(nn.Module):
    def __init__(self, outC):
        ...
        self.resnet_spec = {18: (BasicBlock, [2, 2, 2, 2]),
        ...
        block_class, layers = self.resnet_spec[18]
        self.DetModel = CenterDetNet(block_class, layers, self.ct_heads, head_conv=64)
    ...
    def forward(self, radar_features, cam_features):

        difFeat = self.difAmp(radar_features, cam_features)

        det_ = self.DetModel(difFeat)
        ...
        #
        # det_msk = det_msk.sum(dim=1, keepdim=True).clamp(max=1)
        #
